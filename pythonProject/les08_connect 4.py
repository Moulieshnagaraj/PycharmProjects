...
text=['1.User have to enter only the column number','2.Only two players are allowed','3.Player wins if same coin is get aligned in row,column or an angle']
for i in text:
    time.sleep(2)
    print(i)
...
def isvictorycol():
    ...
    for col in range(sizeoffield):
        ...
        for i in range(1,sizeofcol):
            if fieldboard[col][i-1]!=' ' and fieldboard[col][i]!=' ' and fieldboard[col][i-1]==fieldboard[col][i]:
                counter+=1
            else:
                counter=0
            if counter==3:
...
def isvictoryrow():
    ...
    for row in range(0,6):#(0,6) > 0,1,2,3,4,5
        ...
        for col in range(1,sizeofcol):#(1,7)> 1,2,3,4,5,6
            if fieldboard[col-1][row]!=' ' and fieldboard[col][row]!=" " and fieldboard[col-1][row]==fieldboard[col][row]:
                counter+=1
            else:
                counter=0
            if counter==3:
...
def forwarddiagnol(tile):
    ...
    for c in range(leng-2):
        for r in range(size-4):
            if fieldboard[c][r]==tile and fieldboard[c+1][r+1]==tile and fieldboard[c+2][r+2]==tile and fieldboard[c+3][r+3]==tile:
                state=True
                return True
    return state
...
while True:
    ...
    if isvictorycol():
        board(fieldboard)
        cprint('\nX wins!','green','on_cyan',attrs=['bold'])
        break
    if isvictoryrow():
        board(fieldboard)
        cprint('\nX wins!', 'green', 'on_cyan', attrs=['bold'])
    ...
    if isvictorycol():
        board(fieldboard)
        cprint('\nO wins!','blue','on_grey',attrs=['bold'])
        break
    if isvictoryrow():
        board(fieldboard)
        cprint('\nO wins!', 'blue', 'on_grey', attrs=['bold'])
    ...
    if forwarddiagnol(o):
        board(fieldboard)
        cprint('\nO wins!', 'green', 'on_grey', attrs=['bold'])
        break

    if backwarddiagnol(o):
        board(fieldboard)
        cprint('\nO wins!', 'green', 'on_grey', attrs=['bold'])
        break